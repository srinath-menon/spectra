...
class GalleryHandler(http.server.SimpleHTTPRequestHandler):
    def do_POST(self):
        """Handle Favorite requests - Baseline Move Logic."""
        if self.path == '/favorite':
            ...
            try:
                ...
                # CHANGE: Changed from shutil.copy2 to shutil.move
    ...
    def generate_gallery_html(self):
        ...
        <head>
            ...
            <style>
                ...
                header {{ background: #1a1a1a; padding: 15px; position: sticky; top: 0; z-index: 100; border-bottom: 1px solid #333; display: flex; align-items: center; }}
                h1 {{ margin: 0; cursor: pointer; font-size: 1.2rem; flex-grow: 1; }}
                #folder-toggle {{ background: #333; border: 1px solid #444; color: white; padding: 8px 12px; border-radius: 4px; cursor: pointer; }}
                #folder-drawer {{ display: none; background: #111; padding: 20px; border-bottom: 1px solid #333; max-height: 400px; overflow-y: auto; }}
                ...
                #lightbox {{
                    ...
                    width: 100vw; height: 100vh; background: #000; align-items: center; justify-content: center;
                }}
                #lightbox img {{ width: 100%; height: 100%; object-fit: contain; }}
                .close {{ position: absolute; top: 20px; right: 20px; font-size: 30px; cursor: pointer; z-index: 1010; }}
                #fav-btn {{
                    position: absolute; bottom: 40px; right: 40px; font-size: 40px;
                    background: rgba(255,255,255,0.2); border-radius: 50%; width: 70px; height: 70px;
                    display: flex; align-items: center; justify-content: center; cursor: pointer; z-index: 1010;
                }}
        ...
        <body>
            <header>
                <h1 onclick="resetFilter()">🖼️ LAN Gallery</h1>
